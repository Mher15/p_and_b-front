...
export const CoApplicantData = () => {
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    axios
      .post(`${import.meta.env.VITE_API_URL}/api/coapplicant`, formData)
      .then((response) => {
  ...
  return (
    <div className="coapplicant-container">
      <div className="coapplicant-header">
        <div className="coapplicant-toggle" onClick={toggleFormVisibility}>
          <span>Данные коаппликанта</span>
          <img
            src={
              isFormVisible
                ? "/images/icons/up-arrow.svg"
                : "/images/icons/down-arrow.svg"
            }
            alt=""
          />
        </div>
        <div className="coapplicant-info tooltip top">
          <svg
          ...
          <span>Кто такой коаппликанта</span>
          <span className="tooltip-content">
            Коаппликант — это ваш супруг(а) или близкий родственник
            (родители/дети), обладающий теми же правами по соглашению, что и вы,
            как партнер Best&People
          </span>
        </div>
      </div>

      {isFormVisible && (
        <form className="coapplicant-form" onSubmit={handleSubmit}>
